...
$$
\hat u_i = Y_i- \hat\beta_1-\hat\beta_2X_i
$$
...
(@eq1)
$$
\sum\hat u_i^2 =\sum (Y_i- \hat\beta_1-\hat\beta_2X_i)^2
$$


$$
\sum\hat u_i^2 =f(\hat\beta_1,\hat\beta_2) 
$$


Diferenciando ([@eq1]) se obtiene:
$$
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
datos=read.csv(file="Tabla5_9.csv",sep=";",dec=".",header=TRUE)
attach(datos)
names(datos) 
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
dirdat <- "~/Documents/Consultorias/SEE/2daEscuela/Datos/5e_data_sets"
dirplot <- "~/Documents/Consultorias/SEE/2daEscuela/Slides/[2] Regresion Lineal Simple"

# RL
datos=read.csv(file="Table 31_3.csv",sep=";",dec=".",header=TRUE)
attach(datos)
```
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
datos = read.csv(file="tabla_6_4.csv",sep=";",dec=".",header=TRUE)
attach(datos)
names(datos)
```
...
datos <- read.csv("Table 6_1.csv",sep=";",dec=".",header=TRUE)
attach(datos)
```
...
Abrir los datos `hprice1.xls`. Correr os siguientes modelos e interpretarlos:
...
```{r,echo=FALSE,message=FALSE,warning=FALSE}
datos <- read.csv(file="tabla_8_9.csv",sep=";",dec=".",header=TRUE)
attach(datos)
names(datos)
```
...
- Hay varias formas de hacer la prueba, la mas facil es mediante variables dicotómicas
